...
router.get("/getdata", function (req, res) {
  const footprint_sql = "SELECT name, address, longitude, latitude FROM footprint;";
  const trends_sql = "SELECT users.avatar, users.`name` AS 'title', trends.time, trends.tags, trends.content, trends.img_list as 'images' FROM trends LEFT JOIN users ON trends.user_id = users.id;";
  const menu_sql =
